"""
Local SAM2 Inference on SNMOT-143 Test Sequence

Loads the fine-tuned SAM2 model and runs inference on SNMOT-143,
...
def get_args():
    ...
    parser.add_argument("--sequence", type=str, default="SNMOT-143")
    return parser.parse_args()

args = get_args()

# ── Paths ────────────────────────────────────────────────────
SEQ_DIR   = rf"D:\sebastian\Tesis\VM_FOOTBALL\data\tracking\SoccerNet\tracking\test\test\{args.sequence}"
...
OUTPUT_VIDEO = os.path.join(OUTPUT_DIR, f"{args.sequence}_sam2_finetuned.mp4")
