...
## Agent changelog
- **2026-07-05 (e)** — Map2D render: overlays de video SIEMPRE, aunque no haya calibración. Regresión introducida por el fix de huecos (2026-07-05 b): todo el dibujado de Pass 2 estaba dentro de `if H_inv is not None:` → durante los ~10 s sin calibración de SNMOT-148 el video quedaba "pelado" (sin cajas/dorsales/balón). Corregido en `tactical_vision_2d_mapper.py`: cajas+labels+balón sobre el VIDEO se dibujan siempre (no dependen de la calibración); solo la proyección métrica (minimapa + filas CSV) requiere H_inv, y el minimapa muestra "SIN CALIBRACION (plano no visible)" en esos frames. CSV idéntico al anterior. SNMOT-148 regenerado y verificado frame a frame dentro del hueco.
