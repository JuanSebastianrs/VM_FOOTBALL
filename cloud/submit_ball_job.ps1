...
$JOB_NAME      = "yolo-ball-$($GPU.ToLower())-$TIMESTAMP"
...
# ============================================================
GCS_OUTPUT="gs://PLACEHOLDER_BUCKET/models/yolo_ball_experiments"
...
# ---- Phase 4: Dataset ----
# train_cloud.py handles GCS download to local SSD internally
# (FUSE was too slow for YOLO's 42K label scan: 22 files/s = 32 min)
echo '[4/5] Dataset will be downloaded by train_cloud.py...'
...
if ($status -eq "JOB_STATE_SUCCEEDED") {
    ...
    gcloud storage cp -r "gs://$DATA_BUCKET/models/yolo_ball_experiments/*" "$localResults\"
    ...
    gcloud storage cp "gs://$DATA_BUCKET/models/yolo_ball_experiments/CRASH_LOG.txt" "$crashDir\" 2>$null
